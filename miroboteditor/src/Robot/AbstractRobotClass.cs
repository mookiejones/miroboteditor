...
using SplashScreen = miRobotEditor.Forms.SplashScreen;

namespace miRobotEditor.Robot
{
    public abstract class AbstractRobotClass : IDisposable
    {
       public AbstractRobotClass Instance { get; set; }

        public abstract BackupFile GetFile(FileInfo file);
        ...
        protected AbstractRobotClass()
        {
        }

        protected AbstractRobotClass(FileInfo file)
        {
            _file = file;
        ...
        internal abstract TYPROBOT RobotType { get; }
        ...
        {   
        	return text.Trim().IndexOf(CommentChar).Equals(0);
        }
        ...
        public static IEnumerable<RobotFold> CreateFoldingHelper(ITextSource document, string startFold, string endFold,
                                                                 bool defaultclosed)
        {
                    ...
                    if (document.GetCharAt(offs) != ' ')
                    {
                        ...
                        if (text.ToLower().StartsWith(endFold) && startOffsets.Count > 0)
                        {
                            ...
                            if (valid)
                            {
                                ...
                                var nf = new RobotFold(start, offs + text.Length,
        ...
        protected static Collection<RobotFold> AddInternalVariables(string text, string regex)
        {
            var Return = new Collection<RobotFold>();
            Collection<string> result = GetMatches(text, regex);

            for (int i = 0; i < result.Count - 1; i++)
            {
                Return.Add(new RobotFold {StartOffset = Convert.ToInt32(result[i]), Name = "UserDefined"});
            }
            return Return;
        }
        ...
        internal void PositionVariables(string source, string data)
        {
            ...
            // Get Positions
            Collection<string> v = GetMatches(source + data, regex);
            GroupCollection t = GetMatchCollection(source + data, regex);
        }
