...
namespace miRobotEditor.Core.Services
{
    ...
    public static class FileService
	{
		...
		static void ParserServiceLoadSolutionProjectsThreadEnded(object sender, System.EventArgs e)
		{
				...
				try {
					if (isDirectory && Directory.Exists(oldName)) {
						...
						if (!overwrite && Directory.Exists(newName)) {
							MessageService.ShowMessage("There is an Error with the file in use");
							return false;
						}
