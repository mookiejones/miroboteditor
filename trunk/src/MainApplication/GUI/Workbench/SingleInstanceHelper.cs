...
using FileService = miRobotEditor.Core.Services.FileService;
using FileUtility = miRobotEditor.Classes.FileUtility;
    ...
    public static class SingleInstanceHelper
    {
        ...
        public static bool OpenFilesInPreviousInstance(string[] fileList)
        {
            ...
            try
            {
                ...
                var alternatives = new List<IntPtr>();
                ...
                return alternatives.Any(hWnd => NativeMethods.SendMessage(hWnd, CUSTOM_MESSAGE, new IntPtr(number), new IntPtr(1)).ToInt64() == RESULT_FILES_HANDLED);
            }
            finally
            {
        ...
        internal static IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            if (openEvenIfProjectIsOpened == 0 )
            {
            ...
            try
            {
                WorkbenchSingleton.SafeThreadAsyncCall(
                    () => NativeMethods.SetForegroundWindow(WorkbenchSingleton.MainWin32Window.Handle)
                    );
                string tempFileName = Path.Combine(Path.GetTempPath(), "sd" + fileNumber + ".tmp");
                foreach (string file in File.ReadAllLines(tempFileName))
                {
                    WorkbenchSingleton.SafeThreadAsyncCall(
                        openFileName => FileService.OpenFile(openFileName)
                        , file
                        );
                }
            }
