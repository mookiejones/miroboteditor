...
using FileName = miRobotEditor.Core.FileName;
...
namespace miRobotEditor.WPFPresentation.Controls
{
    ...
    public partial class EditorControl : TextEditor ,ITextEditor
    {
        public IDocument HostControl { get; set; }
        ...
        void UpdateLineTransformers()
        {
            ...
            var textEditorOptions = Options as EditorControl.TextEditorOptions;
            if (textEditorOptions != null && textEditorOptions.HighlightCurrentLine)
                TextArea.TextView.BackgroundRenderers.Add(new HighlightLineRenderer(Document.GetLineByOffset(CaretOffset)));
        }
        ...
        {
            var filename = String.Format("miRobotEditor.Core\\Language\\SyntaxHighlighting.{0}Highlight.xshd", name);
        ...
        {
            Register("KUKA", Languages.KUKA.Ext);
        ...
        {
            var d = new GotoDialog { Editor = this };
        }
        ...
        protected override void OnOptionChanged(System.ComponentModel.PropertyChangedEventArgs e)
        {
        ...
        public void UpdateVisualText()
        {
            ...
            {
                var syn = HighlightingManager.Instance.GetDefinition(HostControl.FileLanguage.RobotType.ToString());
        ...
        {
            if (HostControl.FileLanguage == null | HostControl.FileLanguage is LanguageBase) return;

            var currentword = FindWord();
            // Dont Show Completion window until there are 3 Characters
            if ((String.IsNullOrEmpty(currentword)) | (currentword.Length < 3)) return;

            completionWindow = new CompletionWindow(TextArea);

            HostControl.FileLanguage.CompletionList(currentword, completionWindow.CompletionList.CompletionData);
        ...
        public void SelectText(string text)
        {
            ...
            var d = HostControl.TextBox.Document.GetLineByOffset(Text.IndexOf(text, 0, StringComparison.Ordinal));
            TextArea.Caret.BringCaretToView();
            CaretOffset = d.Offset;
        ...
        private string GetCurrentWord(TextViewPosition? pos)
        {
        ...
        {
            e.CanExecute = (_foldingManager != null && _foldingManager.AllFoldings.Count() > 0);
        }
