...
namespace miRobotEditor.GUI.Editor
{
    ...
    public partial class Editor : TextEditor
    {
        ...
        private void FindBookmarkMembers()
        {
            ...
            if (DummyDoc.ActiveEditor.Robot is Robot.KUKA)
        ...
        public void UpdateVisualText()
        {
              ...
              if (Parent.Robot != null && (!(Parent.Robot is RobotBase)))
              {
                  var syn = HighlightingManager.Instance.GetDefinition(Parent.Robot.RobotType.ToString());
      ...
      //      this.Adapter.JumpTo(line, column);
        ...
        private void RaiseUpdate(object sender, DependencyPropertyChangedEventArgs e)
        {
            if (IsVisible)
            {
                ...
                if (Parent.Robot is RobotBase | Parent.Robot == null)
                    return;
                FindBookmarkMembers();
                UpdateFolds();             
            }
        ...
        private void UpdateFolds()
        {
            ...
            if (File!=null&&File.Extension == ".xml")
                ...
                if (Parent.Robot != null)
                    _foldingStrategy = Parent.Robot.FoldingStrategy;
        ...
        {
            if ((Parent.Robot is RobotBase)&&(File.Extension == ".xml")) return;

            foreach (var section in _foldingManager.AllFoldings)
            {
                section.Title = Parent.Robot.FoldTitle(section, Document);
            }
        }
        ...
        private bool _prompting = true;

        private bool PromptForReload(DateTime lwt)
        {
         
            if (lwt.Year.Equals(lastRead.Year))
                if (lwt.Month.Equals(lastRead.Month))
                    if (lwt.Day.Equals(lastRead.Day))
                        if (lwt.Hour.Equals(lastRead.Hour))
                            if (lwt.Minute.Equals(lastRead.Minute))
                                if (lwt.Second.Equals(lastRead.Second)|_prompting)
                                    return false;
            return true;
        }
        /// <summary>
        ...
        void _watcher_Changed(object sender, FileSystemEventArgs e)
        {
            ...
            if (PromptForReload(lastWriteTime))
            {
                _prompting = true;
                if (FileWatcher.ReloadFile(e.FullPath))
                RaiseReload();
