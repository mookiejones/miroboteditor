...
{
    public class KUKA : AbstractRobotClass
    {
        /// <summary>
        /// Constructor
        /// </summary>
        ...
        internal override Enums.TYPROBOT RobotType { get { return Enums.TYPROBOT.KUKA; } }
       ...
       public KUKA()
        ...
        private readonly List<string> VarTypes =  new List<string> { "E6POS", "E6AXIS", "BOOL", "REAL", "STRUC", "INT", "SIGNAL", "POS"};
        ...
        /*        public new bool ContextPromptBeforeOpen(ILexemLine lexemLine)
                {
                    // get Lexem
                    Lexem = RobotBase.GetLexem(lexemLine);
        ...
        {
        	get{
        		return new string[] { ".dat", ".src", ".ini", ".sub", ".zip", ".kfd" };
        	}
        }
        ...
        private static Controls.Editor GetPointsFromArray(Controls.Editor editor, Collection<string> points)
        {
        ...
        public static Controls.Editor ReversePath(Controls.Editor editor)
        {
  ...
  //         return RobotBase.AddInternalVariables(text, vars);
  //     }
        ...
        public sealed class FileInfo
        {
            ...
            public FileInfo GetFileInfo()
            {
                ...
                param = getinfo(PARAM);
                user = getinfo(USER);
                Visible = access.IndexOf("V", System.StringComparison.Ordinal) > -1;
                return this;
            }
            ...
            private string getinfo(string Lookfor)
            {
                ...
                int found = _text.IndexOf(Lookfor, System.StringComparison.Ordinal);
                if (found > -1)
                    return _text.Substring(found + length, _text.IndexOf("\n", found, System.StringComparison.Ordinal) - found - length);
        ...
        public class RegionFoldingStrategy : AbstractFoldingStrategy
        {
            ...
            /// Create <see cref="NewFolding"/>s for the specified document.
            /// </summary>
            public IEnumerable<RobotFold> CreateNewFoldings(ITextSource document)
            {
                var newFoldings = new List<RobotFold>();
        ...
        const RegexOptions ro = (RegexOptions)3;

        public override Classes.BackupFile GetFile(System.IO.FileInfo file)
        {
            switch (file.Extension)
            {
                case ".src":
                    GetInfo(file.FullName);
                       return new BackupFile() {File = file, Icon = Utilities.LoadBitmap(Global.imgSRC)};
                case ".dat":
                        GetInfo(file.FullName);
                        return new BackupFile() {File = file, Icon = Utilities.LoadBitmap(Global.imgDAT)};
                case ".sub":
                case ".kfd":
                    GetInfo(file.FullName);
                    return new BackupFile() {File = file, Icon = Utilities.LoadBitmap(Global.imgSPS)};
            }
            return null;
        }
        ...
        {
            BackupClass.Functions = GetMatches(MethodRegex, file, Utilities.LoadBitmap(Global.imgMethod),"Function");
        }
        ...
        //public override Regex MethodRegex {get {return new Regex("GLOBAL DEFFCT |^DEFFCT |GLOBAL DEF |^DEF |^EXT ",ro);}}
    	 public override Regex MethodRegex {get {return new Regex(@"(GLOBAL DEFFCT|^DEFFCT|GLOBAL DEF|^DEF|^EXT)\s+([\w]*)?\s*([^\(]+)\(([^\)]*)",ro);}}
    	
        public override Regex FieldRegex {	get {return new Regex("(DECL REAL|^REAL|DECL INT|^INT|DECL BOOL|^BOOL) [a-z$_]+",ro);}}
    	
        public override string CommentChar {get{return ";";}}
        public override Regex SignalRegex { get { return new Regex("Signal",ro); } }

        public override Regex XYZRegex { get { return new Regex("(DECL E6POS|^E6POS|DECL E6AXIS|^E6AXIS) [a-zA-Z_0-9]+", ro); } }
        #endregion
    }
    }
