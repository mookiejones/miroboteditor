...
namespace miRobotEditor.GUI.FunctionWindow
{
    ...
    public partial class FunctionWindow : UserControl
    {
        ...
        public event EventHandler<FunctionEventArgs> FunctionClicked;
        ...
        public void UpdateFunctions(String values, Collection<string> items)
        {
            if ((liItems.Items != null))
            {
                ...
                {
                    AddItems(getMatches(values, items[i]));
                }
            }
        ...
        private static Collection<FunctionItem> getMatches(string Text, string matchstring)
        {
            ...
            while (m.Success)
            {
                ...
                Result.Add(new FunctionItem(text, name, returns, parameters));
                m = m.NextMatch();
            }
            return Result;
        }

        private void raiseFunctionClicked(string text)
        {
            if (FunctionClicked != null)
                FunctionClicked(this, new FunctionEventArgs(text));
        }
        ...
        private void Functions_MouseHover(object sender, EventArgs e)
        {
            if (currentItem != null)
            {
                //       liItems.ToolTip = currentItem.Text;
            }
        }
        ...
        private void DoubleClick(object sender, System.Windows.Input.MouseButtonEventArgs e)
        {
            if ((liItems.SelectedItem == null) | (DummyDoc.ActiveEditor == null))
                return;                
                DummyDoc.ActiveEditor.TextBox.SelectText(liItems.SelectedItem.ToString());
        }
