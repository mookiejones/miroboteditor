...
namespace InlineFormParser.Zip
{
	public class ParallelDeflateOutputStream : Stream
	{
		...
		private readonly TraceBits _DesiredTrace = TraceBits.EmitLock | TraceBits.EmitEnter | TraceBits.EmitBegin |
		                                           TraceBits.EmitDone | TraceBits.EmitSkip | TraceBits.Session |
		                                           TraceBits.Compress | TraceBits.WriteEnter | TraceBits.WriteTake;
		...
		public int MaxBufferPairs
		{
			...
			set
			{
				if (value < 4) throw new ArgumentException("MaxBufferPairs", "Value must be 4 or greater.");
		...
		public int BufferSize
		{
			...
			set
			{
				if (value < 1024) throw new ArgumentOutOfRangeException("BufferSize", "BufferSize must be greater than 1024 bytes");
				_bufferSize = value;
			}
		}
		...
		public override void Write(byte[] buffer, int offset, int count)
		{
			...
			if (count != 0)
			{
				...
				while (true)
				{
					...
					int num2 = workItem.buffer.Length - workItem.inputBytesAvailable > count
						? count
		...
		private void _FlushFinish()
		{
			...
			if (num != 1 && num != 0) throw new Exception("deflating: " + zlibCodec.Message);
		...
		public override void Flush()
		{
			...
			if (!_handlingException) _Flush(false);
		}
		...
		public override void Close()
		{
			...
			{
				_Flush(true);
				if (!_leaveOpen) _outStream.Close();
				_isClosed = true;
			}
		}
				...
				do
				{
					...
					do
					{
						if (Monitor.TryEnter(_toWrite, num2))
						{
							...
							try
							{
								if (_toWrite.Count > 0) num3 = _toWrite.Dequeue();
							}
							finally
							{
		...
		private void _DeflateOne(object wi)
		{
			...
			try
			{
				...
				lock (_latestLock)
				{
					if (workItem.ordinal > _latestCompressed) _latestCompressed = workItem.ordinal;
				}
			...
			catch (Exception pendingException)
			{
				lock (_eLock)
				{
					if (_pendingException != null) _pendingException = pendingException;
				}
			}
		}
