...
namespace InlineFormParser.Zip
{
	...
	public class CRC32
	{
		...
		public int GetCrc32AndCopy(Stream input, Stream output)
		{
			...
			if (output != null) output.Write(array, 0, num);
			TotalBytesRead += num;
			while (num > 0)
			{
		...
		private void GenerateLookupTable()
		{
			...
			{
				uint num = b;
				for (byte b2 = 8; b2 > 0; b2 = (byte) (b2 - 1)) num = (num & 1) != 1 ? num >> 1 : (num >> 1) ^ dwPolynomial;
				if (reverseBits)
		...
		public void Combine(int crc, int length)
		{
			...
			if (length != 0)
			{
				...
				{
					gf2_matrix_square(array, array2);
					if ((num3 & 1) == 1) num = gf2_matrix_times(array, num);
					num3 >>= 1;
					if (num3 == 0) break;
					gf2_matrix_square(array2, array);
					if ((num3 & 1) == 1) num = gf2_matrix_times(array2, num);
					num3 >>= 1;
				} while (num3 != 0);
