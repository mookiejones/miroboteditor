...
namespace InlineFormParser.Zip
{
	internal sealed class DeflateManager
	{
		...
		{
			for (int i = 0; i < InternalConstants.L_CODES; i++) dyn_ltree[i * 2] = 0;
			for (int j = 0; j < InternalConstants.D_CODES; j++) dyn_dtree[j * 2] = 0;
			for (int k = 0; k < InternalConstants.BL_CODES; k++) bl_tree[k * 2] = 0;
		...
		internal void pqdownheap(short[] tree, int k)
		{
			...
			{
				if (num2 < heap_len && _IsSmaller(tree, heap[num2 + 1], heap[num2], depth)) num2++;
				if (_IsSmaller(tree, num, heap[num2], depth)) break;
				heap[k] = heap[num2];
				k = num2;
			}
		...
		internal static bool _IsSmaller(short[] tree, int n, int m, sbyte[] depth)
		{
			...
			if (num >= num2)
			{
				if (num == num2) return depth[n] <= depth[m];
				return false;
			}
		...
		internal int build_bl_tree()
		{
			...
			while (num >= 3 && bl_tree[Tree.bl_order[num] * 2 + 1] == 0) num--;
		...
		internal void send_all_trees(int lcodes, int dcodes, int blcodes)
		{
			...
			for (int i = 0; i < blcodes; i++) send_bits(bl_tree[Tree.bl_order[i] * 2 + 1], 3);
		...
		internal bool _tr_tally(int dist, int lc)
		{
			...
			if ((last_lit & 0x1FFF) == 0 && compressionLevel > CompressionLevel.Level2)
			{
				...
				for (int i = 0; i < InternalConstants.D_CODES; i++)
					num = (int) (num + dyn_dtree[i * 2] * (5L + Tree.ExtraDistanceBits[i]));
				num >>= 3;
				if (matches < last_lit / 2 && num < num2 / 2) return true;
			}

			if (last_lit != lit_bufsize - 1) return last_lit == lit_bufsize;
			return true;
		}
		...
		internal void set_data_type()
		{
			...
			int num = 0;
			int num2 = 0;
			for (; i < 7; i++) num2 += dyn_ltree[i * 2];
			for (; i < 128; i++) num += dyn_ltree[i * 2];
			for (; i < InternalConstants.LITERALS; i++) num2 += dyn_ltree[i * 2];
		...
		{
			int num = 65535;
			if (num > pending.Length - 5) num = pending.Length - 5;
			while (true)
			{
				if (lookahead <= 1)
				{
				...
				flush_block_only(false);
				if (_codec.AvailableBytesOut != 0) continue;
			...
			{
				if (flush != FlushType.Finish) return BlockState.NeedMore;
				return BlockState.FinishStarted;
			}

			if (flush != FlushType.Finish) return BlockState.BlockDone;
		...
		private void _fillWindow()
		{
			while (true)
			{
				...
				if (_codec.AvailableBytesIn != 0)
				{
					...
					if (lookahead >= MIN_LOOKAHEAD) break;
					if (_codec.AvailableBytesIn == 0) break;
					continue;
				}
		...
		internal BlockState DeflateFast(FlushType flush)
		{
			...
			while (true)
			{
				...
				if ((long) num != 0 && ((strstart - num) & 0xFFFF) <= w_size - MIN_LOOKAHEAD &&
				    compressionStrategy != CompressionStrategy.HuffmanOnly) match_length = longest_match(num);
				bool flag;
		...
		internal BlockState DeflateSlow(FlushType flush)
		{
			...
			while (true)
			{
				...
				{
					if (compressionStrategy != CompressionStrategy.HuffmanOnly) match_length = longest_match(num);
				...
				{
					if (_tr_tally(0, window[strstart - 1] & 0xFF)) flush_block_only(false);
					strstart++;
					lookahead--;
					if (_codec.AvailableBytesOut != 0) continue;
					return BlockState.NeedMore;
		...
		internal int longest_match(int cur_match)
		{
			...
			if (prev_length >= config.GoodLength) num >>= 2;
			if (niceLength > lookahead) niceLength = lookahead;
			do
			{
				int num7 = cur_match;
				if (window[num7 + num3] == b2 && window[num7 + num3 - 1] == b && window[num7] == window[num2] &&
				    window[++num7] == window[num2 + 1])
				{
					num2 += 2;
					num7++;
					while (window[++num2] == window[++num7] && window[++num2] == window[++num7] && window[++num2] == window[++num7] &&
					       window[++num2] == window[++num7] && window[++num2] == window[++num7] && window[++num2] == window[++num7] &&
					       window[++num2] == window[++num7] && window[++num2] == window[++num7] && num2 < num6)
					{
					}
		...
		internal int End()
		{
			if (status != INIT_STATE && status != BUSY_STATE && status != FINISH_STATE) return -2;
			pending = null;
			head = null;
			prev = null;
			window = null;
			if (status != BUSY_STATE) return 0;
			return -3;
		}
		...
		internal int SetParams(CompressionLevel level, CompressionStrategy strategy)
		{
			...
			if (compressionLevel != level)
			{
				...
				if (config.Flavor != this.config.Flavor && _codec.TotalBytesIn != 0) result = _codec.Deflate(FlushType.Partial);
				compressionLevel = level;
				this.config = config;
				SetDeflater();
			}
		...
		internal int Deflate(FlushType flush)
		{
			    ...
			    (status != FINISH_STATE || flush == FlushType.Finish))
			{
				...
				if (status == INIT_STATE)
				{
					...
					if (num3 > 3) num3 = 3;
					num2 |= num3 << 6;
					if (strstart != 0) num2 |= PRESET_DICT;
					num2 += 31 - num2 % 31;
					status = BUSY_STATE;
				...
				if (_codec.AvailableBytesIn != 0 || lookahead != 0 || flush != 0 && status != FINISH_STATE)
				{
					...
					if (blockState == BlockState.FinishStarted || blockState == BlockState.FinishDone) status = FINISH_STATE;
					switch (blockState)
					{
						case BlockState.NeedMore:
						case BlockState.FinishStarted:
							if (_codec.AvailableBytesOut == 0) last_flush = -1;
							return 0;
						case BlockState.BlockDone:
							...
							_codec.flush_pending();
							if (_codec.AvailableBytesOut != 0) break;
							last_flush = -1;
							return 0;
					}
				}
