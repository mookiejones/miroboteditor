...
using DMC_Engineering.Core;
using DMC_Engineering.Core.WinForms;
using DMC_Engineering.miRobotEditor.Gui;

namespace DMC_Engineering.miRobotEditor.Sda
{
	/// <summary>
	/// Form used to display display unhandled errors in miRobotEditor.
	/// </summary>
	public class ExceptionBox : Form
	{
		...
		static void ShowErrorBox(Exception exception, string message, bool mustTerminate)
		{
			...
			try {
				...
				using (ExceptionBox box = new ExceptionBox(exception, message, mustTerminate)) {
					if (DMC_Engineering.miRobotEditor.Gui.WorkbenchSingleton.InvokeRequired)
						box.ShowDialog();
					else
						box.ShowDialog(DMC_Engineering.miRobotEditor.Gui.WorkbenchSingleton.MainWin32Window);
				}
			} catch (Exception ex) {
		...
		{
			if (MessageBox.Show(StringParser.Parse("${res:DMC_Engineering.miRobotEditor.ExceptionBox.QuitWarning}"), MessageService.ProductName, MessageBoxButtons.YesNo, MessageBoxIcon.Question, MessageBoxDefaultButton.Button2, MessageBoxOptions.DefaultDesktopOnly)
			    == DialogResult.Yes)
			{
		...
		void InitializeComponent()
		{
			...
			this.closeButton.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.ExitSharpDevelop}";
			...
			this.label3.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.ThankYouMsg}";
			// 
			// label2
			// 
			...
			this.label2.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.HelpText2}";
			// 
			// label
			// 
			...
			this.label.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.HelpText1}";
			// 
			// continueButton
			// 
			...
			this.continueButton.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.Continue}";
			...
			this.reportButton.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.ReportError}";
			...
			this.copyErrorCheckBox.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.CopyToClipboard}";
			// 
			// exceptionTextBox
			// 
			...
			this.Text = "${res:DMC_Engineering.miRobotEditor.ExceptionBox.Title}";
